...
Для выполнения задачи Г, код сервера не сохранил обратную совместимость с задачами А, Б, а так же был переписан в ООП стиле
Поэтому если есть необходимость посмотреть исходный код из задач А, Б, его можно найти в старых коммитах 
по [этой ссылке](https://github.com/Seva-Vaskin/networks-course-homeworks/blob/04b41518f1034d57bdd0c7a900d786296e2d27e6/lab03/server/server.py).
...
#### Демонстрация работы
С флагом `--concurrent` работает так же, но создавая потоки для каждого запроса
![](images/task_B_1.png)
![](images/task_B_2.png)
![](images/task_B_3.png)
...
#### Демонстрация работы
Для демонстрации работы в функцию обработки запроса была добавлена команда `time.sleep(1)`, 
что имитирует некую долгую работу. В приведённом ниже тесте (`concurrency_level=2`) можно видеть, 
что при запуске 3-х запросов одновременно 2 запроса работаю ~1 секунду, а 3-й ~2 секунды. 
Что доказывает, что очередь действительно работает. 
...
#### Решение
Сначала должен собраться полноценный пакет размера 56 * 8 бит со скоростью 128 * 1000 бит/с. 
Есть один бит уже есть, то до полной сборки пакета остаётся $\frac{58\cdot 8 - 1}{128 \cdot 1000}$ секунд. 

Далее пакет из 56 * 8 бит отправится по каналу от A к B со скоростью 10^6 бит/c, то есть 
пакет будет идти $\frac{56 \cdot 8}{10^6}$ секунд.

И ещё надо учесть задержку распространения 0.005 с

Ответ: $\frac{56\cdot 8 - 1}{128 \cdot 1000} + \frac{56 \cdot 8}{10^6} + 0.005 = 0.0089401875$
...
#### Решение
$N = 11$ (10 в буфере и 1 на передаче)
$d = d_1 + d_2 = 0.01 + 1/100 = 0.02$ ($100$ пакетов в секунду, значит пакет "летит" 1/100 секунды). 
$N = a \cdot d \implies a = \frac{N}{d} = \frac{11}{0.02} = 550$ пакетов / с 
### Задача 3 (2 балла)
Рассмотрим рисунок.
