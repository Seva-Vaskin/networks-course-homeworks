...
## Реализация протокола Stop and Wait (8 баллов) - Done
...
#### Демонстрация работы
Логи запусков можно найти тут: 
* [client.log](reliable_udp/client.log)
* [server.log](reliable_udp/server.log)
...
#### Демонстрация работы
Выполнил, смотреть логи, приложенные выше
...
#### Решение
Средняя пропускная способность TCP (capacity) $CAP = \dfrac{1.22 \cdot MSS}{RTT \sqrt{L}} 
\implies L = \left( \dfrac{1.22 \cdot MSS}{CAP \cdot RTT} \right)^2$

Тогда $T$ можно выразить следующим образом $T = \dfrac{\frac{1}{L} \cdot MSS}{CAP} 
= \dfrac{RTT^2}{1.22^2 \cdot MSS} \cdot CAP$ - функция от $CAP$
...
#### Решение
Через $n$ шагов размер пакета будет $\frac{W}{2}\cdot (1 + a)^n = W 
\implies 2 = (1 + a)^n \implies n = \log_{1 + a}(2)$

Общее кол-во сегментов $S = \frac{W}{2} \sum_{i = 0}^n (1 + a)^i = \frac{W}{2} \cdot \frac{(a + 1)^(n + 1) -  1}{a}$

Тогда $L = \frac{1}{S} = \frac{2a}{W \cdot ((a + 1)^(\log_{1 + a}(2) + 1) - 1)} = \frac{2a}{W \cdot (2(a + 1) - 1)}$
