...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - ru-RU,ru,en-US,en
   ![](./1.2.png)
   - Информация о браузере: User-Agent, принимаемые типы файлов: Accept, принимаемые типы кодировок: Accept-encoding
...
3. Сколько сегментов TCP, содержащих данные, потребовалось для передачи одного HTTP ответа?
   - 1 (насколько я понял, в моём случае разбиение не произошло)
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Нет, так как судя по последовательности номеров запросов, сначала был сделан GET запрос и получен ответ для pearson.png, и только потом, был отправлен следующий GET запрос для 8E_cover_small.jpg. В случае параллельного скачивания, логично было бы увидеть оба GET запроса, а после оба ответа. 
